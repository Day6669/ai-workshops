...
;=================================================================
;=================================================================
to-report TERMINAL-TEST[board] ;; tests whether the game is over or not
  if has-any-player-won? (evaluate board) or not moves-left? (board) [report true]
  report false
end

to-report GET-ACTIONS [board] ;; gets the usable actions on the board
 let coords (list ) ;; uses a foreach loop to loop through rows and cols to check for if any actions are avaliable
  foreach [0 1 2 ] [ row ->
    foreach [0 1 2] [col ->
      if item row item col board = "_" [
        set coords lput (list row col) coords ;; creates a board for the list of actions
      ]
    ]
  ]
  report coords
...
to-report RESULT [board action player] ;; a new board to apply the action
 let new_board replace-in-board (item 0 action) (item 1 action) board player
 report new_board
...
to-report MAX_VALUE[board depth] ;; This is the function for the max player score
  if TERMINAL-TEST board [
    report (calculate-utility (evaluate board) depth)
  ]
  let v -1000
  foreach GET-ACTIONS board [action ->
   let state RESULT board action "x"
   set v max(list v (MIN_VALUE state (depth + 1))) ;; make a new board for each action and then return the highest score.
  ]
    report v
end

to-report MIN_VALUE[board depth] ;; The function for minimum player score.
   if TERMINAL-TEST board [
    report (calculate-utility (evaluate board) depth) ;; uses calculate-utility to help workout the overall evaluation of the board.
  ]
  let v 1000
  foreach GET-ACTIONS board [action ->
   let state RESULT board action "o"
   set v min(list v (MAX_VALUE state (depth + 1))) ;; make a new board for each action and then return the lowest score.
  ]
    report v
end

to-report minimax [board] ;; MINIMAX search alogrithm
  let goodscore -1000
  let bestaction (list -1 -1)

  foreach GET-ACTIONS board [ action -> ;; checks the score for each action and then returns the best possible score.
    let state RESULT board action "x"
    let newscore MIN_VALUE state 0
    if newscore > goodscore [
      set goodscore newscore
